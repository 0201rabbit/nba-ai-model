...
def fetch_nba_lite(game_date_str): 
    # 核心修正：統一使用 NBA API 最接受的 MM/DD/YYYY 格式
    ...
    try:
        # 🛡️ 修正點：game_date 必須使用 date_api (MM/DD/YYYY)
        sb = scoreboardv2.ScoreboardV2(game_date=date_api, headers=headers, timeout=15)
        games_df = sb.get_data_frames()[0]
        line_score = sb.get_data_frames()[1]
        
        # 如果第一種格式失敗，嘗試 ISO 格式作為 Fallback
        if games_df.empty:
            sb = scoreboardv2.ScoreboardV2(game_date=game_date_str, headers=headers, timeout=15)
            games_df = sb.get_data_frames()[0]
            line_score = sb.get_data_frames()[1]

        time.sleep(1.2) # 增加微小延遲防止被封鎖
...
# 3. UI 介面
# ---------------------------------------------------------
st.set_page_config(page_title="NBA AI V32 修正版", layout="wide", page_icon="🏀") 
...
with st.spinner("啟動穩定引擎：讀取賽事與數據..."): 
...
if games_df.empty: 
    st.warning(f"📅 找不到 {formatted_date} 的賽程。")
    st.info("提示：若確定今日有比賽，請嘗試重新整理頁面，或檢查是否為 NBA API 伺服器高峰期。")
else:
    ...
    if total_finished > 0:
        st.sidebar.divider()
        st.sidebar.metric("🎯 本日策略命中率", f"{(hit_count/total_finished):.1%}")

    st.divider() 
    st.header("🔍 蒙地卡羅深度解析儀") 
    if match_data:
        s_g = st.selectbox("選擇深入分析場次：", match_data, format_func=lambda x: x["對戰組合"]) 
...
st.caption("NBA AI V32.1 - 修正日期請求邏輯：已恢復賽程抓取穩定性。")
