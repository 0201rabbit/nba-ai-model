...
def fetch_live_odds(api_key, target_date_str):
    ...
    try:
        ...
        # 🔑 抓取 API 回傳的標頭，記錄剩餘次數
        quota_remaining = r.headers.get('x-requests-remaining', '未知')
        quota_used = r.headers.get('x-requests-used', '未知')
...
if os.path.exists("api_quota.json"):
    with open("api_quota.json", "r", encoding="utf-8") as f:
        ...
        st.sidebar.caption(f"最後更新: {quota_data.get('last_updated', '未知')}")
else:
...
with st.spinner("啟動 10,000 次蒙地卡羅迴圈模擬與期望值運算中..."): 
    ...
    # 呼叫抓取盤口功能，並帶入日期供建立快取檔名
    live_odds = fetch_live_odds(api_key, formatted_date) if target_date == (datetime.now() - timedelta(hours=8)).date() else {}
